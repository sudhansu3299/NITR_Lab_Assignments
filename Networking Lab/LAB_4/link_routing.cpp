...
int linkStateRouting(Network &network,int src,int dest){
    int time = 0;
    while(src != dest){
        network.updateRoutingTable(src,time);
        cout<<"packet is at router number: "<<src<<endl;
...
int main(){
    ...
    int LSR_Delay = linkStateRouting(network,src,dest);
    cout<<"End to end delay for Link State Routing is "<<LSR_Delay<<" units"<<endl;
}
